...
class TagRulebook < Struct.new(:deleteion_number, :rules)
	def next_string(string)
		rule_for(string).follow(string).slice(deleteion_number..-1)
	end
	...
	def applies_to?(string)
		!rule_for(string).nil? && string.length >= deleteion_number
	end
	...
	def cyclic_padding_rules(encoder)
		Array.new(encoder.alphabet.length, CyclicTagRule.new("")) * (deleteion_number - 1)
	end
